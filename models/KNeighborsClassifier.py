...
class KNeighborsClassifierModel:
    """
   A class used to represent a K-Nearest Neighbors Classfier Model

   Attributes
   ----------
   model :
       an instance of the KNeighborsClassifier Class from sklearn
   ...
   """
    ...
    def __init__(self, n_neighbors=100, weights='uniform', algorithm='auto', leaf_size=30, p=2, metric='minkowski'):
        ...
        n_neighbors=100:
            ...
            generalizable predictions by considering more samples around each query point. A higher number of neighbors 
            can smooth out the effects of noisy labels or outliers, leading to more robust predictions.
        
        weights='uniform':
            Each neighbor contributes equally to the classification, regardless of their distance to the query point.
            Uniform weights are simpler and involve fewer assumptions about the data, which can be beneficial if there's no 
            clear rationale for using distance-based weighting.
             
        algorithm='auto':
            This setting allows scikit-learn to automatically choose the most efficient algorithm based on the dataset's structure, 
            leading to potentially faster computations without needing to manually test each algorithm.
